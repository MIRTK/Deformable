...
def recon_cortical_surface(name, mask=None, regions=None,
    ...
    if not mask and hemisphere == Hemisphere.Both:
        rname = recon_cortical_surface('{}-{}{}'.format(base, hemi2str(Hemisphere.Right), ext),
                                       ...
                                       remove_spikes=remove_spikes, temp=temp)
    ...
    with ExitStack() as stack:
        ...
        if corpus_callosum_mask:
            ...
            out3 = out2
        out4 = push_output(stack, clean_cortical_surface(out3, remove_spikes=remove_spikes))
        remove_intersections(out4, oname=name, max_attempt=10)
    return name
...
def join_cortical_surfaces(name, regions, right_mesh, left_mesh, bs_cb_mesh=None,
    ...
    with ExitStack() as stack:
        ...
        if check:
            ...
            if num_boundaries != 0:
                cleaned = push_output(stack, clean_cortical_surface(joined, remove_duplicates=True, remove_boundaries=True))
                info = evaluate_surface(cleaned, mesh=True, topology=True)
                num_boundaries = get_num_boundaries(info)
                if num_boundaries != 0:
